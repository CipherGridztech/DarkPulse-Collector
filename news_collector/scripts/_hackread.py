...
class _hackread(leak_extractor_interface, CrawlerBase, ABC):
    _instance = None
    ...
    def _store_raw_card(self, card: news_model) -> str:
        aid = self.store_raw_card_generic("HACKREAD:raw", card)
    ...
    def parse_leak_data(self) -> dict:
        print("[HACKREAD] run() → Playwright first, then requests fallback (centralized)")
        # Collect links across deterministic pagination using the base helper per-page (keeps behaviour)
        ...
        def _parse_and_store(page, link, idx, total):
            try:
                print(f"[HACKREAD] Visiting [{idx}/{total}]: {link}")
                ...
                print(f"[HACKREAD] ✅ Parsed: {title[:90]} | Author: {author or '(n/a)'} | Date: {date_raw or '(n/a)'} | AID: {aid}")
                return True
            except Exception as ex:
    ...
    def _nlp_enrich_and_store(self):
        # delegate to generic implementation in base
        self._nlp = nlp
